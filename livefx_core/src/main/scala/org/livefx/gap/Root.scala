...
final case class Root[+A](child: Tree[A] = Leaf0)(implicit vg: A => Int) {
  ...
  final def insert[B >: A](index: Int, value: B)(implicit vg: B => Int): Root[B] = Root(child.insert(index, value) match {
    case Branch4(a, b, c, d) => Branch2(Branch2(a, b), Branch2(c, d))
    case Leaf4(a, b, c, d) => Branch2(Leaf2(a, b), Leaf2(c, d))
    case newChild => newChild
  })

  final def update[B >: A](index: Int, value: B)(implicit vg: B => Int): Root[B] = Root(child.update(index, value))

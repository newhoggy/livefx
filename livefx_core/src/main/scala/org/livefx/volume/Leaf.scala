...
trait Leaf[+A] extends Tree[A] {
  override def insert[B >: A](index: Int, value: B)(implicit hm: HasMonoid[B, Int]): Leaf[B]
  override def takeCount(count: Int)(implicit hm: HasMonoid[A, Int]): Leaf[A]
  override def dropCount(count: Int)(implicit hm: HasMonoid[A, Int]): Leaf[A]
}

final case class Leaf0() extends Leaf[Nothing] {
  ...
  final override def insert[B](index: Int, value: B)(implicit hm: HasMonoid[B, Int]): Leaf[B] = index match {
    case 0 => Leaf1(value)
    case _ => throw new IndexOutOfBoundsException
  }
  
  final override def update[B](index: Int, value: B)(implicit hm: HasMonoid[B, Int]): Leaf[B] = throw new IndexOutOfBoundsException
...
final case class Leaf1[+A, V](a: A)(implicit hm: HasMonoid[A, Int]) extends Leaf[A] {
  ...
  final override def takeCount(count: Int)(implicit hm: HasMonoid[A, Int]): Leaf[A] = count match {
    case 0 => Leaf0()
    case 1 => this
    case _ => throw new IndexOutOfBoundsException
  }

  final override def dropCount(count: Int)(implicit hm: HasMonoid[A, Int]): Leaf[A] = count match {
    case 0 => this
    case 1 => Leaf0()
    case _ => throw new IndexOutOfBoundsException
  }

  final override def insert[B >: A](index: Int, value: B)(implicit hm: HasMonoid[B, Int]): Leaf[B] = index match {
    case 0 => Leaf2(value, a)
    case 1 => Leaf2(a, value)
    case _ => throw new IndexOutOfBoundsException
  }

  final override def update[B >: A](index: Int, value: B)(implicit hm: HasMonoid[B, Int]): Leaf[B] = index match {
    case 0 => Leaf1(value)
    case _ => throw new IndexOutOfBoundsException
  }

  final override def remove(index: Int)(implicit hm: HasMonoid[A, Int]): (A, Tree[A]) = index match {
    case 0 => (a, Leaf0())
    case _ => throw new IndexOutOfBoundsException
  }
}

final case class Leaf2[+A](a: A, b: A)(implicit hm: HasMonoid[A, Int]) extends Leaf[A] {
  ...
  final override def insert[B >: A](index: Int, value: B)(implicit hm: HasMonoid[B, Int]): Leaf[B] = {
    index match {
...
final case class Leaf3[+A](a: A, b: A, c: A)(implicit hm: HasMonoid[A, Int]) extends Leaf[A] {
...
final case class Leaf4[+A](a: A, b: A, c: A, d: A)(implicit hm: HasMonoid[A, Int]) extends Leaf[A] {
...
final case class LeafN[+A](vs: List[A], size: Int)(implicit hm: HasMonoid[A, Int]) extends Leaf[A] {
  assert(vs.size == size)
  final override val volume: Int = vs.foldLeft(0)(_ + hm.monoidOf(_))
  
  final override def count: Int = size

  final override def takeCount(count: Int)(implicit hm: HasMonoid[A, Int]): Leaf[A] = Leaf(vs.take(count), count)

  final override def dropCount(count: Int)(implicit hm: HasMonoid[A, Int]): Leaf[A] = Leaf(vs.drop(count), this.count - count)
  ...
  final override def update[B >: A](index: Int, value: B)(implicit hm: HasMonoid[B, Int]): Leaf[B] = {
...
final object LeafN {
  def apply[A](vs: List[A])(implicit hm: HasMonoid[A, Int]): LeafN[A] = LeafN(vs, vs.size)
}

final object Leaf {
  def apply[A](vs: List[A], count: Int)(implicit hm: HasMonoid[A, Int]): Leaf[A] = {
